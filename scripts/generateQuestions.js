// Script to generate and insert questions into Supabase for Uncover app
...
// Function to generate SQL insert statements for questions instead of direct insertion
const generateSQLForQuestions = (questions) => {
  let sql = '-- SQL script to insert questions into Supabase content table\n';
  sql += 'INSERT INTO content (content, type, category) VALUES\n';
  sql += questions.map(q => `(\ '${q.content.replace(/'/g, '\'\'')}\ ', \ 'questions\ ', \ '${q.category}\ ')`).join(',\n');
  sql += ';';
  return sql;
};
...
const main = async () => {
  ...
  for (const category of categories) {
    ...
    console.log(`Generating SQL for 100 questions for ${category}...`);
    fullSQL += `\n\n-- Questions for ${category}\n`;
    fullSQL += generateSQLForQuestions(questions);
  }
  console.log('Writing SQL to file...');
  require('fs').writeFileSync('./fullQuestionsInsert.sql', fullSQL);
  console.log('SQL script generated at ./fullQuestionsInsert.sql. Please copy and paste this into the Supabase SQL Editor.');
};
