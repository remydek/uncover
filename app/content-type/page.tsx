      ...
      <div className="space-y-6">
          {contentTypes.map((type) => (
            <button
              ...
              className={`w-full ${type.color} text-white p-6 rounded-lg transition-all duration-200 hover:scale-105 active:scale-95 flex items-center justify-between`}
            >
              <div className="text-center flex-1">
                <div className="text-xl font-semibold mb-1">{type.name}</div>
                <div className="text-sm opacity-90">{type.description}</div>
              </div>
              <ChevronDown className="w-5 h-5 ml-4 opacity-70" />
            </button>
          ))}
      </div>
